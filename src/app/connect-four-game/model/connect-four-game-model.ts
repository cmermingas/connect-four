...
export class ConnectFourGameModel {
  ...
  static fromJSON(jsonData: any): ConnectFourGameModel {
    ...
    result.columns = jsonData.columns.map(item => item.slice());
  ...
  toJSON(): any {
    return {
      ...
      columns: this.columns.map(item => item.slice()),
  ...
  copy(): ConnectFourGameModel {
    ...
    result.columns = this.columns.map(item => item.slice());
  ...
  playAtColumn(col: number): boolean {
    let result = false;
    const cell = this.columnIndex[col];
